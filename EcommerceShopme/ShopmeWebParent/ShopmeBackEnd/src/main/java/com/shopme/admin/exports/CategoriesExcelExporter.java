package com.shopme.admin.exports;
...
public class CategoriesExcelExporter extends AbstractExporter {
	...
	private void writeDataLines(List<Category> listCategories) {
		...
		for (Category categories : listCategories) {
			...
			createCell(row, columnIndex++, categories.getId(), cellStyle);
			createCell(row, columnIndex++, categories.getName(), cellStyle);
			createCell(row, columnIndex++, categories.getAlias(), cellStyle);
			createCell(row, columnIndex++, categories.isEnabled(), cellStyle);
		}
	}
}
